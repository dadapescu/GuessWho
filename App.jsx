...
export default function App() {
  ...
  const tabMatches = matches.filter(m=>m.phase===tab).sort((a,b)=>new Date(a.kickoff)-new Date(b.kickoff));
        ...
        <>
          ...
          <div style={{display:"flex",gap:8,marginTop:14}}>
            {["1","X","2"].map((v,i)=>{
              ...
              return (
                <button key={v} onClick={()=>!locked&&onPick(match.id,v)} disabled={locked}
                  style={{flex:1,padding:"12px 0",borderRadius:10,fontSize:15,fontWeight:700,cursor:locked?"default":"pointer",border:`2px solid ${active?C.navy:C.line}`,background:active?C.navy:"#fff",color:active?"#fff":locked?C.line:C.ink,transition:"all .15s"}}>
                  <div>{v}</div>
