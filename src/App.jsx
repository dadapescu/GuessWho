...
import { generateKO, computeStandings, GROUP_LETTERS } from "./bracket";
...
const LOCK_OFFSET = 0; // pariuri deschise pana la fluierul de start

// ── Clasament grupe (folosit doar in GroupStandings component) ──
...
export default function App() {
  ...
  // ── Salveaza rezultat meci KO (admin) ──
  const saveKoResult = async (koId, result, home, away) => {
    const next = { ...koResults };
    if (result === null) delete next[koId];
    else next[koId] = { ...result, home, away };
    setKoResults(next);
    try { await saveState({ koResults: next }); showToast("Salvat."); }
    catch { showToast("Eroare."); }
  };

  // ── Salveaza override manual (admin) ──
  ...
  // ── Toate meciurile: grupe + eliminatorii generate ──
  const koMatches = useMemo(() => generateKO(matches, koResults, overrides), [matches, koResults, overrides]);
  const allMatches = useMemo(() => [...matches, ...koMatches], [matches, koMatches]);

  useEffect(() => {
    if (allMatches.length === 0) return;
    const ORDER = ["grupe","16imi","optimi","sferturi","semi","finala"];
    for (const phaseId of ORDER) {
      const pm = allMatches.filter(m => m.phase === phaseId);
      if (pm.length === 0) continue;
      if (!pm.every(m => m.result)) { setTab(phaseId); return; }
    }
    setTab("finala");
  }, [allMatches.length]);
  ...
  // ── Meciul urmator ──
  const nextMatch = allMatches.filter(m=>!m.result&&!m.isPlaceholder&&new Date(m.kickoff).getTime()>now)
  ...
  // O "zi de meci" incepe la ora 15:00 si se termina la 14:59 a zilei calendaristice urmatoare.
  // Eticheta zilei de meci pentru un meci = data calendaristica la care apartine conceptual.
  const matchDayLabel = (d) => {
    const dt = new Date(d);
    if (dt.getHours() < 15) dt.setDate(dt.getDate()-1);
    dt.setHours(0,0,0,0);
    return dt;
  };
  const todayLabel = new Date(now); todayLabel.setHours(0,0,0,0);
  const yesterdayLabel = new Date(todayLabel); yesterdayLabel.setDate(yesterdayLabel.getDate()-1);
  const isOld = (m) => m.result && matchDayLabel(m.kickoff) < yesterdayLabel;

  const PHASE_ORDER = ["grupe","16imi","optimi","sferturi","semi","finala"];
  const isPhaseComplete = (id) => { const pm=allMatches.filter(m=>m.phase===id); return pm.length>0&&pm.every(m=>m.result); };
  const sortedPhases = [...PHASES].sort((a,b)=>{ if(a.id==="rezultate"||b.id==="rezultate") return 0; const ai=PHASE_ORDER.indexOf(a.id),bi=PHASE_ORDER.indexOf(b.id); if(ai===-1||bi===-1) return 0; const ac=isPhaseComplete(a.id)?1:0,bc=isPhaseComplete(b.id)?1:0; if(ac!==bc) return ac-bc; return ai-bi; });
  const tabMatches = (tab === "grupe" ? matches : allMatches).filter(m=>m.phase===tab).sort((a,b)=>{
    const ao=isOld(a)?1:0, bo=isOld(b)?1:0;
    if(ao!==bo) return ao-bo;
    const ac=!!a.result?1:0, bc=!!b.result?1:0;
    ...
    <Shell>
      <header style={{background:`linear-gradient(135deg,${C.navyD},${C.navy})`,color:"#fff",padding:"18px 16px 0"}}>
        <div style={{maxWidth:760,margin:"0 auto"}}>
          ...
          <nav style={{display:"flex",gap:4,marginTop:10,overflowX:"auto"}}>
            {[...sortedPhases,{id:"clasament",label:"🏆 Clasament"},...(isAdmin?[{id:"admin",label:"⚙ Arbitru"}]:[])]
              .map(p=>(
      ...
      <main style={{maxWidth:760,margin:"0 auto",padding:"20px 16px 60px"}}>
        {tab==="clasament" ? <Leaderboard standings={standings} finishedCount={finished.length} me={me}/> :
         ...
         tab==="admin" ? <AdminPanel matches={matches} onSave={saveMatches} onResetAll={resetAll} koMatches={koMatches} koResults={koResults} onSaveKo={saveKoResult} overrides={overrides} onOverride={saveOverride} now={now}/> :
         tabMatches.length===0 ? <div style={st.empty}>Niciun meci la {phaseLabel(tab)} inca.</div> :
         tabMatches.map(m=><MatchCard key={m.id} match={m} me={me} myPick={myPicks[m.id]||null} usersData={usersData} users={users} now={now} onPick={placeBet} isKo={m.phase!=="grupe"}/>)}
      </main>
...
// ── Card meci ───────────────────────────────────────────────
function MatchCard({match,me,myPick,usersData,users,now,onPick,isKo}) {
      ...
      {!done && (
        <>
          ...
          <div style={{display:"flex",gap:8,marginTop:14}}>
            {(isKo?["1","2"]:["1","X","2"]).map((v,i)=>{
              const labels=isKo?[match.home,match.away]:([match.home,"egal",match.away]);
  ...
  return (
    <>
      <div style={{...st.card,background:"#E8EBF5"}}>
        <div style={{fontSize:13,color:C.navy,lineHeight:1.5}}>📊 <strong>Clasamentele celor 12 grupe</strong>, actualizate dupa fiecare meci inchis. Top 2 + cele mai bune 8 locuri 3 trec in 16-imi.</div>
      </div>
      {GROUPS.map(g=>{
...
// ── Admin ───────────────────────────────────────────────────
function AdminPanel({matches,onSave,onResetAll,koMatches,koResults,onSaveKo,overrides,onOverride,now}) {
  ...
  const unclosed=[...matches,...(koMatches||[])].filter(m=>
    !m.result && !m.isPlaceholder && new Date(m.kickoff).getTime()+2*60*60*1000<=Date.now()
  );

  const getScore=(id)=>scoreRefs.current[id]?scoreRefs.current[id].value:"";
  const setRes=(id)=>{const p=picks[id];if(!p)return;onSave(matches.map(m=>m.id===id?{...m,result:{pick:p,score:getScore(id).trim()}}:m));};
  const clrRes=(id)=>onSave(matches.map(m=>m.id===id?{...m,result:undefined}:m));
  const setKoRes=(m)=>{const p=picks[m.id];if(!p)return;onSaveKo(m.id,{pick:p,score:getScore(m.id).trim()},m.home,m.away);};
  const clrKoRes=(id)=>onSaveKo(id,null,null,null);

  const sortM=(arr)=>[...arr].sort((a,b)=>{const ac=!!a.result?1:0,bc=!!b.result?1:0;if(ac!==bc)return ac-bc;return new Date(a.kickoff)-new Date(b.kickoff);});
  const isComplete=(arr)=>arr.length>0&&arr.every(m=>m.result);

  const phases=[
    {id:"16imi",label:"16-imi",matches:[...(koMatches||[])].filter(m=>m.phase==="16imi"&&!m.isPlaceholder),isKo:true},
    {id:"optimi",label:"Optimi",matches:[...(koMatches||[])].filter(m=>m.phase==="optimi"&&!m.isPlaceholder),isKo:true},
    {id:"sferturi",label:"Sferturi",matches:[...(koMatches||[])].filter(m=>m.phase==="sferturi"&&!m.isPlaceholder),isKo:true},
    {id:"semi",label:"Semifinale",matches:[...(koMatches||[])].filter(m=>m.phase==="semi"&&!m.isPlaceholder),isKo:true},
    {id:"finala",label:"Finala",matches:[...(koMatches||[])].filter(m=>m.phase==="finala"&&!m.isPlaceholder),isKo:true},
    {id:"grupe",label:"Faza grupelor",matches:matches,isKo:false},
  ].sort((a,b)=>{const ac=isComplete(a.matches)?1:0,bc=isComplete(b.matches)?1:0;return ac-bc;});

  const togglePhase=(id)=>setCollapsed(p=>({...p,[id]:!p[id]}));
  ...
  return (
    <>
      ...
      <div style={{...st.card,background:"#E8EBF5"}}>
        <div style={{fontSize:13,color:C.navy,lineHeight:1.5}}><strong>Tu esti arbitrul.</strong> 1/X/2 + scor la grupe, 1/2 la eliminatorii. Fazele terminate se strang automat.</div>
      </div>

      {phases.map(phase=>{
        const done=isComplete(phase.matches);
        const isOpen=collapsed[phase.id]===undefined?!done:!collapsed[phase.id];
        const sorted=sortM(phase.matches);
        const openCount=sorted.filter(m=>!m.result).length;
        if(phase.matches.length===0) return null;
        return (
          <div key={phase.id} style={{...st.card,...(done?{opacity:0.8}:{})}}>
            <div onClick={()=>togglePhase(phase.id)}
              style={{display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}}>
              <h2 style={{...st.h2,fontSize:18}}>
                {phase.label}
                {done&&<span style={{fontSize:12,fontWeight:500,color:C.green,marginLeft:10,textTransform:"none"}}>✓ complet</span>}
                {!done&&openCount>0&&<span style={{fontSize:12,fontWeight:500,color:C.soft,marginLeft:10,textTransform:"none"}}>{openCount} neinchise</span>}
              </h2>
              <span style={{fontSize:16,color:C.soft}}>{isOpen?"▴":"▾"}</span>
            </div>
            {isOpen&&sorted.map(m=>(
              <div key={m.id} style={{borderTop:`1px solid ${C.line}`,padding:"12px 0"}}>
                <div style={{fontSize:14}}><strong>{flag(m.home)} {m.home} – {m.away} {flag(m.away)}</strong>
                  <div style={{fontSize:12,color:C.soft}}>{m.group||phase.label} · {fmtDate(m.kickoff)}{m.result&&<strong style={{color:C.navy}}> · Final: {m.result.pick} {m.result.score}</strong>}</div>
                </div>
                {!m.result?(
                  <div style={{display:"flex",gap:8,marginTop:10,alignItems:"center",flexWrap:"wrap"}}>
                    {(phase.isKo?["1","2"]:["1","X","2"]).map(v=>(
                      <button key={v} onClick={()=>setPicks(p=>({...p,[m.id]:v}))}
                        style={{...st.btnSmall,background:picks[m.id]===v?C.navy:"#fff",color:picks[m.id]===v?"#fff":C.ink}}>{v}</button>
                    ))}
                    <input ref={el=>scoreRefs.current[m.id]=el} style={{...st.input,width:90,padding:"8px 10px"}} placeholder="scor 2-1"/>
                    <button style={st.btnSmallP} onClick={()=>phase.isKo?setKoRes(m):setRes(m.id)}>Inchide meciul</button>
                    {!phase.isKo&&<input type="datetime-local" style={{...st.input,width:190,padding:"8px 10px"}} value={m.kickoff}
                      onChange={e=>onSave(matches.map(x=>x.id===m.id?{...x,kickoff:e.target.value}:x))}/>}
                  </div>
                ):(<button style={st.btnGhost} onClick={()=>phase.isKo?clrKoRes(m.id):clrRes(m.id)}>anuleaza rezultatul</button>)}
              </div>
            ))}
          </div>
        );
      ...
      <div style={st.card}>
        ...
        <p style={{fontSize:12,color:C.soft,margin:"6px 0 12px"}}>Daca FIFA schimba o echipa. Slot: "W73", "W74" etc.</p>
        {Object.entries(overrides).length>0&&(
          <div style={{marginBottom:12}}>
            {Object.entries(overrides).map(([slot,team])=>(
              <div key={slot} style={{display:"flex",justifyContent:"space-between",padding:"6px 10px",background:C.goldP,borderRadius:8,marginBottom:4,fontSize:13}}>
                <span><strong>{slot}</strong> → {team}</span>
                <button onClick={()=>onOverride(slot,null)} style={st.btnGhost}>sterge</button>
              </div>
            ))}
          </div>
        )}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr auto",gap:8}}>
          <input style={st.input} placeholder="Slot (ex: W73)" value={overrideSlot} onChange={e=>setOverrideSlot(e.target.value)}/>
          <input style={st.input} placeholder="Echipa" value={overrideTeam} onChange={e=>setOverrideTeam(e.target.value)}/>
          <button style={st.btnSmallP} onClick={()=>{if(overrideSlot&&overrideTeam){onOverride(overrideSlot,overrideTeam);setOverrideSlot("");setOverrideTeam("");}}}>Adauga</button>
        </div>
      </div>
