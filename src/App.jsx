...
export default function App() {
  ...
  // Tab-uri sortate: fazele terminate se duc la coada
  ...
  const isPhaseComplete = (phaseId) => {
    ...
    return pm.length > 0 && pm.every(m=>m.result);
  };
  const sortedPhases = [...PHASES].sort((a,b)=>{
    if(a.id==="rezultate"||b.id==="rezultate") return 0;
    const ai=PHASE_ORDER.indexOf(a.id), bi=PHASE_ORDER.indexOf(b.id);
    if(ai===-1||bi===-1) return 0;
    const ac=isPhaseComplete(a.id)?1:0, bc=isPhaseComplete(b.id)?1:0;
