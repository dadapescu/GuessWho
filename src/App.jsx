...
function AdminPanel({matches,onSave,onResetAll,koMatches,koResults,onSaveKo,overrides,onOverride,now}) {
  ...
  const sortedGroup=[...matches].sort((a,b)=>{
    const aClosed=!!a.result?1:0, bClosed=!!b.result?1:0;
  ...
  const setKoRes=(m)=>{
    const d=draft[m.id];
    if(!d||!d.pick)return;
    onSaveKo(m.id,{pick:d.pick,score:(d.score||"").trim()},m.home,m.away);
  };
  const clrKoRes=(id)=>onSaveKo(id,null,null,null);
  const sortedKO=[...(koMatches||[])].filter(m=>!m.isPlaceholder).sort((a,b)=>{
    const aClosed=!!a.result?1:0, bClosed=!!b.result?1:0;
    if(aClosed!==bClosed) return aClosed-bClosed;
    return new Date(a.kickoff)-new Date(b.kickoff);
  });

  const MatchRow = ({m, isKo, draft, setDraft, onClose, onClear}) => (
      ...
      {!m.result?(
        <div style={{display:"flex",gap:8,marginTop:10,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <input style={{...st.input,width:90,padding:"8px 10px"}} placeholder="scor 2-1"
            value={(draft[m.id]||{}).score||""}
            onChange={e=>{const v=e.target.value;setDraft(p=>({...p,[m.id]:{...(p[m.id]||{}),score:v}}));}}/>
          <button style={st.btnSmallP} onClick={onClose}>Inchide meciul</button>
          {!isKo&&<input type="datetime-local" style={{...st.input,width:190,padding:"8px 10px"}} value={m.kickoff}
            onChange={e=>onClear(e.target.value)}/>}
        </div>
      ):(
        <button style={st.btnGhost} onClick={onClear}>anuleaza rezultatul</button>
      )}
    </div>
  );
  ...
  return (
    <>
      ...
      <div style={{...st.card,background:"#E8EBF5"}}>
        <div style={{fontSize:13,color:C.navy,lineHeight:1.5}}><strong>Tu esti arbitrul.</strong> Inchizi meciurile din grupe cu 1/X/2 + scor, cele eliminatorii cu 1 sau 2 (fara egal). Bracket-ul se construieste automat.</div>
      </div>

      <div style={st.card}>
        <h2 style={st.h2}>Faza grupelor ({sortedGroup.length})</h2>
        {sortedGroup.map(m=><MatchRow key={m.id} m={m} isKo={false} draft={draft} setDraft={setDraft}
          onClose={()=>setRes(m.id)}
          onClear={(v)=>v?onSave(matches.map(x=>x.id===m.id?{...x,kickoff:v}:x)):clrRes(m.id)}/>)}
      </div>

      {sortedKO.length>0&&(
        <div style={st.card}>
          <h2 style={st.h2}>Eliminatorii ({sortedKO.length})</h2>
          {sortedKO.map(m=><MatchRow key={m.id} m={m} isKo={true} draft={draft} setDraft={setDraft}
            onClose={()=>setKoRes(m)}
            onClear={()=>clrKoRes(m.id)}/>)}
        </div>
      )}
