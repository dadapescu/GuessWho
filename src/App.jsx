...
export default function App() {
  ...
  const [tab, setTab] = useState("grupe"); // va fi actualizat dupa ce se incarca datele
  ...
  // Auto-detecteaza faza activa: prima faza care nu e complet terminata
  useEffect(() => {
    ...
    for (const phaseId of PHASE_ORDER) {
      ...
      // Daca faza are meciuri neinchise sau e in desfasurare, mergi acolo
      if (!pm.every(m => m.result)) { setTab(phaseId); return; }
    }
    // Toate fazele terminate - mergi la finala
    setTab("finala");
  }, [allMatches.length]); // doar la prima incarcare cu date
...
function AdminPanel({matches,onSave,onResetAll,koMatches,koResults,onSaveKo,overrides,onOverride,now}) {
  ...
  const sortedGroup=[...matches].sort((a,b)=>{const ac=!!a.result?1:0,bc=!!b.result?1:0;if(ac!==bc)return ac-bc;return new Date(a.kickoff)-new Date(b.kickoff);});
  const sortedKO=[...(koMatches||[])].filter(m=>!m.isPlaceholder).sort((a,b)=>{const ac=!!a.result?1:0,bc=!!b.result?1:0;if(ac!==bc)return ac-bc;return new Date(a.kickoff)-new Date(b.kickoff);});
  ...
  return (
    <>
      ...
      <div style={{...st.card,background:"#E8EBF5"}}>
        <div style={{fontSize:13,color:C.navy,lineHeight:1.5}}><strong>Tu esti arbitrul.</strong> Inchizi meciurile din grupe cu 1/X/2 + scor, cele eliminatorii cu 1 sau 2. Bracket-ul se construieste automat.</div>
      </div>
      <div style={st.card}>
        <h2 style={st.h2}>Faza grupelor ({sortedGroup.length})</h2>
        {sortedGroup.map(m=>(
          <div key={m.id} style={{borderTop:`1px solid ${C.line}`,padding:"12px 0"}}>
            <div style={{fontSize:14}}><strong>{flag(m.home)} {m.home} – {m.away} {flag(m.away)}</strong>
              <div style={{fontSize:12,color:C.soft}}>{m.group} · {fmtDate(m.kickoff)}{m.result&&<strong style={{color:C.navy}}> · Final: {m.result.pick} {m.result.score}</strong>}</div>
            </div>
            {!m.result?(
              <div style={{display:"flex",gap:8,marginTop:10,alignItems:"center",flexWrap:"wrap"}}>
                {["1","X","2"].map(v=>(<button key={v} onClick={()=>setPicks(p=>({...p,[m.id]:v}))} style={{...st.btnSmall,background:picks[m.id]===v?C.navy:"#fff",color:picks[m.id]===v?"#fff":C.ink}}>{v}</button>))}
                ...
                <input type="datetime-local" style={{...st.input,width:190,padding:"8px 10px"}} value={m.kickoff} onChange={e=>onSave(matches.map(x=>x.id===m.id?{...x,kickoff:e.target.value}:x))}/>
              </div>
      ...
      {sortedKO.length>0&&(
        <div style={st.card}>
          <h2 style={st.h2}>Eliminatorii ({sortedKO.length})</h2>
          {sortedKO.map(m=>(
            <div key={m.id} style={{borderTop:`1px solid ${C.line}`,padding:"12px 0"}}>
              <div style={{fontSize:14}}><strong>{flag(m.home)} {m.home} – {m.away} {flag(m.away)}</strong>
                <div style={{fontSize:12,color:C.soft}}>{m.group} · {fmtDate(m.kickoff)}{m.result&&<strong style={{color:C.navy}}> · Final: {m.result.pick} {m.result.score}</strong>}</div>
              ...
              {!m.result?(
                <div style={{display:"flex",gap:8,marginTop:10,alignItems:"center",flexWrap:"wrap"}}>
                  {["1","2"].map(v=>(<button key={v} onClick={()=>setPicks(p=>({...p,[m.id]:v}))} style={{...st.btnSmall,background:picks[m.id]===v?C.navy:"#fff",color:picks[m.id]===v?"#fff":C.ink}}>{v}</button>))}
                  <input ref={el=>scoreRefs.current[m.id]=el} style={{...st.input,width:90,padding:"8px 10px"}} placeholder="scor 2-1"/>
                  <button style={st.btnSmallP} onClick={()=>setKoRes(m)}>Inchide meciul</button>
                </div>
              ):(<button style={st.btnGhost} onClick={()=>clrKoRes(m.id)}>anuleaza rezultatul</button>)}
            </div>
          ))}
        </div>
      )}
      ...
      <div style={st.card}>
        ...
        <p style={{fontSize:12,color:C.soft,margin:"6px 0 12px"}}>Daca FIFA schimba o echipa. Slot: "1A", "2C", "3CEFHI", "W73" etc.</p>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr auto",gap:8}}>
          <input style={st.input} placeholder="Slot (ex: 1A)" value={overrideSlot} onChange={e=>setOverrideSlot(e.target.value)}/>
