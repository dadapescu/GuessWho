...
export default function App() {
  ...
  const matchDayStart = (d) => {
    const dt = new Date(d);
    if (dt.getHours() < 15) dt.setDate(dt.getDate()-1);
    dt.setHours(15,0,0,0);
    return dt;
  };
  const currentMatchDayStart = matchDayStart(now);
  const cutoffMatchDayStart = new Date(currentMatchDayStart); cutoffMatchDayStart.setDate(cutoffMatchDayStart.getDate()-1);
  const isOld = (m) => m.result && matchDayStart(m.kickoff) < cutoffMatchDayStart;
