    ...
    <div key={m.id} style={{borderTop:`1px solid ${C.line}`,padding:"12px 0"}}>
      ...
      {!m.result?(
        <div style={{display:"flex",gap:8,marginTop:10,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <button style={st.btnSmallP} onClick={()=>isKo?setKoRes(m):setRes(m.id)}>Inchide meciul</button>
        ...
        <button style={st.btnGhost} onClick={()=>isKo?clrKoRes(m.id):clrRes(m.id)}>anuleaza rezultatul</button>
      )}
    </div>
  );
  ...
  return (
    <>
      ...
      <div style={st.card}>
        ...
        {sortedGroup.map(m=><MatchRow key={m.id} m={m} isKo={false}/>)}
      </div>

      {sortedKO.length>0&&(
        <div style={st.card}>
          <h2 style={st.h2}>Eliminatorii ({sortedKO.length})</h2>
          {sortedKO.map(m=><MatchRow key={m.id} m={m} isKo={true}/>)}
        </div>
      )}
